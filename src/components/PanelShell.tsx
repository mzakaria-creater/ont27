...
function SmsRail({ onMinimize }: { onMinimize: () => void }) {
  ...
  useEffect(() => {
    ...
    const pump = window.setInterval(() => { void syncProviders() }, 10_000)
...
export default function PanelShell({ children }: { children: ReactNode }) {
  ...
  const renderLinks = (group: NavGroupId) => BUILT_LINKS.filter((l) => l.group === group && visible(l) && matchesNavQuery(l)).map((l) => (
    <Link key={l.to} to={l.to === '/sms' ? { pathname: '/sms', search: `?from=${new Intl.DateTimeFormat('en-CA', { timeZone: 'Africa/Cairo' }).format(new Date())}&to=${new Intl.DateTimeFormat('en-CA', { timeZone: 'Africa/Cairo' }).format(new Date())}` } : l.to} title={locale === 'en' ? l.labelEn : l.labelAr} onClick={() => setNavOpen(false)} className={`sidebar-item sidebar-link${pathname === l.to ? ' active' : ''}`}>
