...
export default function TransactionEditDialog({ txId, ontargetRef, status, amount, currency, gateway, onDone }: {
  ...
  const { user } = useAuth(); const steward = STEWARD_ROLES.has(user?.role ?? '')
  const [open, setOpen] = useState(false); const [nextStatus, setNextStatus] = useState(''); const [reason, setReason] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null); const [done, setDone] = useState<string | null>(null)
  const changing = nextStatus !== '' && nextStatus !== status
  ...
  const submit = async () => {
    ...
    try {
      ...
      const out = await api<{ executed_on_provider?: boolean; telegram?: { sent: number } }>(endpoint, { method: 'POST', body: JSON.stringify({ status: nextStatus, amount: null, reason: reason.trim() }) })
      ...
      setNextStatus(''); setReason(''); onDone?.()
    } catch (e) { setError(e instanceof ApiError ? e.code : 'edit_failed') }
    finally { setBusy(false) }
  }
  return <>
    <button type="button" className="btn-ghost btn-sm" onClick={() => { setOpen(true); setDone(null); setError(null) }} title="Edit transaction status"><Pencil size={13}/> Edit status</button>
