...
export default function TransactionEditDialog({ txId, ontargetRef, status, amount, currency, gateway, onDone }: {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setNextStatus(''); setNextAmount(''); setReason(''); setReasonChoice(''); onDone?.()
    } catch (e) { setError(e instanceof ApiError ? e.code : 'edit_failed') }
    finally { setBusy(false) }
  }
  return <>
    <button type="button" className="btn-ghost btn-sm" onClick={() => { setOpen(true); setDone(null); setError(null); setNextAmount(''); setReason(''); setReasonChoice('') }} title="Edit transaction"><Pencil size={13}/> Edit</button>
