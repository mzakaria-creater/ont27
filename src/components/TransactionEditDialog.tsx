...
export default function TransactionEditDialog({ txId, ontargetRef, status, amount, currency, gateway, onDone, iconOnly = false }: {
  ...
  const changing = (nextStatus !== '' && nextStatus !== status) || (nextAmount !== '' && Number(nextAmount) !== Number(amount ?? 0)) || nextSenderNumber.trim() !== ''
  ...
  const submit = async () => {
    ...
    try {
      ...
      const out = await api<{ executed_on_provider?: boolean; telegram?: { sent: number } }>(endpoint, { method: 'POST', body: JSON.stringify({ status: nextStatus, amount: canEditAmount && nextAmount !== '' ? Number(nextAmount) : null, sender_number: canEditAmount && nextSenderNumber !== '' ? nextSenderNumber.trim() : null, reason: reason.trim() }) })
      ...
      setNextStatus(''); setNextAmount(''); setNextSenderNumber(''); setReason(''); setReasonChoice(''); onDone?.()
