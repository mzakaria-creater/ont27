...
function DocTable({ headers, rows, codeAll }: { headers: string[]; rows: string[][]; codeAll?: boolean }) {
  const isMobile = useIsMobile()
  if (isMobile) return <div className="risk-card-list">{rows.map((r) => <div key={r[0]} className="risk-row-card">
    <div className="risk-row-card-head"><code>{r[0]}</code></div>
    {r.slice(1).map((v, i) => <div key={i} className="cell-sub"><span className="muted">{headers[i + 1]}: </span>{codeAll ? <code>{v}</code> : v}</div>)}
  </div>)}</div>
  return <div className="table-wrap"><table className="data-table"><thead><tr>{headers.map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{rows.map((r) => <tr key={r[0]}>{r.map((v, i) => <td key={i}>{codeAll || i === 0 ? <code>{v}</code> : v}</td>)}</tr>)}</tbody></table></div>
}
  ...
  return <>
    ...
    <div className="guide-layout">
      ...
      <article className="card guide-doc">
        ...
        <section id="create-payment"><h3>5. Create payment</h3><div className="guide-endpoint"><b>POST</b><code>/checkout/create</code></div><Code>{createExample}</Code><DocTable headers={['Field','Required','Description']} rows={[['merchant_ref','Yes','Unique per merchant'],['amount','Yes','Positive payment amount'],['currency','Yes','EGP, USD, or EUR'],['callback_url','Optional','HTTPS webhook endpoint'],['return_url','Optional','Customer return URL'],['customer_phone','Optional','Customer mobile number'],['description','Optional','Payment description'],['metadata','Optional','Merchant JSON metadata']]} /></section>
