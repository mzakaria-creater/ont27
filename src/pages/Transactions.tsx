            ...
            <table className="data-table all-transactions-table portal-transaction-grid">
              ...
              <tbody>
                {data.rows.map((r) => {
                  ...
                  return (
                    <Fragment key={rowKey}>
                      {r.matched_sms && <tr className={`tx-sms-raw-row tx-sms-above tx-pair-${Math.abs(Number(id ?? 0)) % 5}`}><td colSpan={11}><div className="tx-sms-raw"><span className="tx-sms-label">📨 SMS الخاص بالمعاملة</span><span className="mono">#{r.matched_sms.id}</span><span>{r.matched_sms.sender_name ?? r.matched_sms.sender_number ?? '—'} → {r.matched_sms.receiver_number ?? '—'}</span><span className="mono">{money(r.matched_sms.amount, r.currency ?? 'EGP')}</span><code>{r.matched_sms.raw_sms ?? r.matched_sms.message ?? r.matched_sms.sms_first_line ?? '—'}</code></div></td></tr>}
