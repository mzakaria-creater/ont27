                  ...
                  return (
                    <Fragment key={rowKey}>
                      ...
                      <div className="tx-expanded-sms-side">
                        ...
                        {r.kind === 'deposit' ? (r.matched_sms ? <div className="tx-sms-raw">{r.status === 'DECLINED' ? <span className="tx-sms-label tx-sms-warning-label"><AlertTriangle size={13} aria-hidden="true" /> SMS مع مرفوضة</span> : <span className="tx-sms-label">📨 SMS الخاص بالمعاملة</span>}<span className="mono">#{r.matched_sms.id}</span><span>{r.matched_sms.sender_name ?? r.matched_sms.sender_number ?? '—'} → {r.matched_sms.receiver_number ?? '—'}</span><span className="mono">{money(r.matched_sms.amount, r.currency ?? 'EGP')}</span>{r.matched_sms.balance_after != null && <strong className="tx-live-balance mono">رصيدك الحالي {money(r.matched_sms.balance_after, r.currency ?? 'EGP')}</strong>}<code className={r.status === 'DECLINED' ? 'is-warning-raw' : undefined}>{r.matched_sms.raw_sms ?? r.matched_sms.message ?? r.matched_sms.sms_first_line ?? '—'}</code>
                          {r.matched_sms.match_status === 'auto_review' && !r.matched_sms.sender_number && <div className="tx-sms-weak-nudge"><AlertTriangle size={12} aria-hidden="true"/> {t('مطابقة ضعيفة — SMS بدون رقم هاتف (اسم فقط). راجع صورة الإثبات قبل القرار.', 'Weak match — no phone number in this SMS (name only). Check the proof screenshot before deciding.')}</div>}
                        </div> : <div className="tx-sms-raw is-missing"><strong>{t('لا توجد رسالة SMS مرتبطة', 'No SMS linked')}</strong><Link to={`/sms?amount=${r.amount ?? ''}`}>{t('البحث عن رسالة', 'Search SMS')}</Link></div>) : <div className="tx-sms-raw is-missing"><strong>{t('لا ينطبق على السحوبات', 'Not applicable to payouts')}</strong></div>}
                      </div></div>
                      </td></tr>}
                    </Fragment>
                  )
