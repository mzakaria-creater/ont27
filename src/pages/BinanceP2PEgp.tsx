...
export default function BinanceP2PEgp() {
  ...
  return <PanelShell>
    ...
    <section className="binance-trader-split"><div className="recent-head"><div><h3><Users size={18}/> {t('تقرير مقسم حسب Trader', 'Trader split report')}</h3><span className="cell-sub">{t('كل بطاقة تعرض صفقات هذا الطرف فقط.', 'Each card contains only this trader’s trades.')}</span></div><span className="binance-vf-method"><Smartphone size={14}/> Vodafone Cash · EGP</span></div>{grouped.map(({ summary, rows }) => <article className="card recent-card binance-trader-card" key={summary.trader}><header className="recent-head"><div><h3>{summary.trader}</h3><span className="cell-sub">{summary.orders} {t('صفقة','trades')} · {egp(summary.totalEgp)}</span></div><div className="binance-trader-totals"><span className="positive-text">BUY {egp(summary.buyEgp)}</span><span className="negative-text">SELL {egp(summary.sellEgp)}</span></div></header>{isMobile ? (
        <div className="risk-card-list">
          {rows.map((row, index) => (
            <div key={row.orderNumber ?? `${row.advNo}-${index}`} className="risk-row-card">
              <div className="risk-row-card-head"><span className="mono">{row.orderNumber ?? row.advNo ?? '—'}</span><span className={`pay-status-badge ${row.tradeType === 'SELL' ? 'st-declined' : 'st-paid'}`}>{row.tradeType}</span></div>
              <div className="cell-sub">{row.asset} · {row.amount ?? '—'} · {row.paymentMethod ?? '—'}</div>
              <div className="risk-row-card-foot"><span className="mono">{egp(row.totalPrice)}</span><span className="pay-status-badge st-dim">{row.orderStatus ?? '—'}</span></div>
              <div className="cell-sub mono">{row.createTime ? new Date(row.createTime).toLocaleString('en-US', { timeZone: 'Africa/Cairo' }) : '—'}</div>
            </div>
          ))}
        </div>
      ) : (
