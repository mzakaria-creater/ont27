...
export default function AdminTransactions() {
  ...
  return <PanelShell>
    ...
    <section className="card recent-card admin-trx-card"><div className="table-wrap admin-trx-table-wrap"><table className="data-table admin-trx-table"><thead><tr>{['Action','Our TRX No.','Transaction ID','Status','Payment Type','Amount','User Email','User Phone Number','Sender Account Name','Sender Account Number','Created UTC Date','Modified UTC Date','User Name','Merchant Ref ID','To Account Name','To Account Number','To Bank','Currency'].map((h)=><th key={h}>{h}</th>)}</tr></thead><tbody>
      {rows.map((r)=>{const st=statusMeta(r.status); const open=expanded===r.tx_id; const image=proofUrl(r); return <Fragment key={r.tx_id}>
        <tr className={r.status==='PENDING'?'row-pending':undefined}>
          <td><div className="admin-trx-actions"><button className="btn-ghost btn-sm" onClick={()=>setExpanded(open?null:r.tx_id)}>{open?<ChevronUp size={14}/>:<ChevronDown size={14}/>}</button>{image&&<ProofIconButton compact url={image} onOpen={(url)=>setProof({url,tx:r.tx_id,onApprove:r.status==='PENDING'&&can('deposits','can_approve')?async()=>{await decide(r,'approve');setProof(null)}:undefined,onDecline:r.status==='PENDING'&&can('deposits','can_approve')?async()=>{await decide(r,'decline');setProof(null)}:undefined})}/>} {r.status==='PENDING'&&can('deposits','can_approve')&&<><button className="btn-primary btn-sm" disabled={busy===r.tx_id} onClick={()=>void decide(r,'approve')}>Approve</button><button className="btn-ghost danger btn-sm" disabled={busy===r.tx_id} onClick={()=>void decide(r,'decline')}>Decline</button></>}<TransactionEditDialog txId={r.tx_id} ontargetRef={text(r,'ontarget_ref','tx_id')} status={r.status} amount={r.amount} currency={text(r,'currency')} gateway={text(r,'gateway')} currentReceivingWallet={text(r,'to_account_number') === '—' ? text(r,'receiving_wallet') : text(r,'to_account_number')} onDone={()=>void load()}/></div></td>
          <td className="mono">{text(r,'ontarget_ref')}</td><td className="mono">{r.tx_id}<div><span className={`deposit-kind ${r.deposit_kind === 'retention_deposit' ? 'is-retention' : 'is-first'}`}>{r.deposit_kind === 'retention_deposit' ? '↻ Retention' : '★ First'}</span></div></td><td><span className={`pay-status-badge ${st.cls}`}>{st.label}</span></td><td><MethodLogo method={text(r,'payment_method')}/></td><td className="mono">{money(Number(r.amount??0),text(r,'currency'))}</td><td>{text(r,'email')}</td><td><SenderIdentity name={text(r,'sender_name')} phone={text(r,'sender_number')} /></td><td>{text(r,'sender_name')}</td><td className="mono">{text(r,'sender_account_number','sender_number')}</td><td className="mono">{text(r,'created_utc','first_seen_at')}</td><td className="mono">{text(r,'modified_utc','last_status_change')}</td><td>{rawValue(r,'user_name','UserName','sender_name')}</td><td className="mono">{text(r,'merchant_reference') !== '—' ? text(r,'merchant_reference') : rawValue(r,'Reference1')}</td><td>{text(r,'to_account_name')}</td><td className="mono">{text(r,'to_account_number')}</td><td>{text(r,'to_bank')}</td><td>{text(r,'currency')}</td>
        </tr>
