...
interface Reports { totals: { depCount: number; depVolume: number; declined: number; payCount: number; payVolume: number; commission: number; fees: number }; daily: { date: string; depVolume: number; payVolume: number }[]; byMethod: { key: string; count: number; volume: number }[] }
  ...
  return <PanelShell>
    ...
    {tab === 'overview' && <>
      ...
      <div className="kpi-grid">
        <div className="kpi-card"><div className="kpi-value">{money((reports?.totals.depVolume ?? 0) + (reports?.totals.payVolume ?? 0), 'EGP')}</div><div className="kpi-label">{t('إجمالي الحجم', 'Total volume')}</div><div className="cell-sub">{t('كل البيانات المعتمدة', 'all approved data')}</div></div>
        ...
        <div className="kpi-card"><div className="kpi-value">{money((reports?.totals.commission ?? 0) + (reports?.totals.fees ?? 0), 'EGP')}</div><div className="kpi-label">{t('إيراد الرسوم', 'Revenue from fees')}</div><div className="cell-sub">{t('القيم المسجلة فقط', 'recorded values only')}</div></div>
      </div>
