...
interface Reports { totals: { depCount: number; depVolume: number; declined: number; payCount: number; payVolume: number }; daily: { date: string; depVolume: number; payVolume: number }[] }
  ...
  return <PanelShell>
    ...
    {tab === 'overview' && <>
      ...
      <div className="kpi-grid">
        <div className="kpi-card"><div className="kpi-value">{money(stats?.depositVolume ?? 0, 'EGP')}</div><div className="kpi-label">{t('إيداعات معتمدة', 'Approved deposits')}</div><div className="cell-sub">{stats?.depositCount ?? 0} {t('عملية', 'transactions')}</div></div>
        <div className="kpi-card"><div className="kpi-value">{money(stats?.payoutVolume ?? 0, 'EGP')}</div><div className="kpi-label">{t('سحوبات معتمدة', 'Approved payouts')}</div><div className="cell-sub">{stats?.payoutCount ?? 0} {t('عملية', 'transactions')}</div></div>
        <div className="kpi-card"><div className="kpi-value">{money(net, 'EGP')}</div><div className="kpi-label">{t('صافي التدفق', 'Net flow')}</div><div className="cell-sub">{t('وارد ناقص صادر', 'Incoming less outgoing')}</div></div>
        <Link to="/approvals" className="kpi-card"><div className="kpi-value">{(executive?.queues.pendingDeposits ?? 0) + (executive?.queues.pendingPayouts ?? 0)}</div><div className="kpi-label">{t('قيد المراجعة', 'Awaiting review')}</div><div className="cell-sub">{rate.toFixed(1)}% {t('نسبة القبول', 'approval rate')}</div></Link>
      </div>
    ...
    {tab === 'reports' && <section className="card recent-card"><div className="recent-head"><h3>{t('ملخص التقارير لكل البيانات', 'All-time report summary')}</h3><Link className="pay-status-link" to="/reports">{t('فتح التقارير التفصيلية', 'Open detailed reports')} ←</Link></div><div className="kpi-grid"><div className="kpi-card"><div className="kpi-value">{money(reports?.totals.depVolume ?? 0, 'EGP')}</div><div className="kpi-label">{t('إيداعات معتمدة', 'Approved deposits')}</div></div><div className="kpi-card"><div className="kpi-value">{money(reports?.totals.payVolume ?? 0, 'EGP')}</div><div className="kpi-label">{t('سحوبات معتمدة', 'Approved payouts')}</div></div><div className="kpi-card"><div className="kpi-value">{reports?.totals.depCount ?? 0}</div><div className="kpi-label">{t('عمليات إيداع', 'Deposit transactions')}</div></div><div className="kpi-card"><div className="kpi-value">{reports?.totals.declined ?? 0}</div><div className="kpi-label">{t('إيداعات مرفوضة', 'Declined deposits')}</div></div></div></section>}
  </PanelShell>
}
