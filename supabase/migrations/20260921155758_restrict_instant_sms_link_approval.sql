...
begin
  ...
  -- and actually matches. When either side is unidentified (common for
  -- Orange SMS with no sender_number), the match relied on wallet+timing
  -- alone, which is not reliable enough for an irreversible instant
  -- decision -- fall through to the normal cron path instead, which has
  -- additional review time before it executes on the provider.
