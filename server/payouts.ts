...
payoutRoutes.get('/', requirePerm('payouts', 'can_view'), async (c) => {
  ...
  if (rows.length) {
    const smsIds = rows.map((row) => row.matched_sms_id).filter((id): id is number => id != null)
    const [{ data: decisions }, { data: smsRows }] = await Promise.all([
      db.from('payout_decision_log').select('maven_id, decision, actor_name, remark, executed_on_provider, created_at')
        .in('maven_id', rows.map((row) => row.maven_id)).order('created_at', { ascending: false }),
      smsIds.length ? db.from('inbound_sms').select('id, received_at, amount, receiver_number, wallet_number, provider, trx_id, trx_reference, balance_after, message').in('id', smsIds) : Promise.resolve({ data: [] }),
    ])
  ...
  return c.json({ rows: rows.map((row) => ({ ...presentPayout(row), decision: decisionById.get(row.maven_id) ?? null,
    linked_sms: row.matched_sms_id ? smsById.get(Number(row.matched_sms_id)) ?? null : null })), total: count ?? 0, limit, offset })
})
...
payoutRoutes.get('/:mavenId', requirePerm('payouts', 'can_view'), async (c) => {
  ...
  const [{ data: decisions }, { data: actions }, { data: linkedSms }] = await Promise.all([
    ...
    data.matched_sms_id ? db.from('inbound_sms').select('id, received_at, amount, receiver_number, wallet_number, provider, trx_id, trx_reference, balance_after, message').eq('id', data.matched_sms_id).maybeSingle() : Promise.resolve({ data: null }),
  ])
  return c.json({ payout: { ...data, linked_sms: linkedSms ?? null, decisions: decisions ?? [], actions: actions ?? [] } })
})
...
payoutRoutes.post('/:mavenId/decision', requirePerm('payouts', 'can_approve'), async (c) => {
  ...
  await autoLinkWithdrawalSms(100).catch((error) => console.error('pre-decision WD SMS matcher failed:', error))
  const { data: before, error: readErr } = await db
    .from('maven_payout_transactions')
    .select('maven_id, status, amount, ontarget_ref, merchant, matched_sms_id')
    .eq('maven_id', mavenId)
    .maybeSingle()
  if (readErr) return c.json({ error: 'db_error', detail: readErr.message }, 500)
  if (!before) return c.json({ error: 'not_found' }, 404)
  const { data: linkedSms } = before.matched_sms_id
    ? await db.from('inbound_sms').select('id, trx_id, trx_reference, receiver_number, amount').eq('id', before.matched_sms_id).maybeSingle()
    : { data: null }
  if (decision === 'APPROVED' && !proofUrl && !linkedSms) return c.json({ error: 'payment_proof_required' }, 400)
  ...
  const suppliedUtr = typeof body?.utr_number === 'string' ? body.utr_number.trim().slice(0, 120) : ''
  const utrNumber = suppliedUtr || linkedSms?.trx_id || linkedSms?.trx_reference || ''
